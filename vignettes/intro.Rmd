...
__SC19020__ is a simple R package developed to calculate the covariance and correlation coefficient of two set of samples for the 'Statistical Computing' course. Two functions are considered, namely, _covR_ (compute covariance of the given data) and _corrR_ (compute correlation coefficient of the given data).

## _covR_
The source R code for _covR_ is as follows:
```{r,eval=FALSE}
covR <- function(X, Y) {
  n <- length(X)
...
In order to show how to use _covR_, we give an example.
...
## _corrR_
The source R code for _corrR_ is as follows:
```{r,eval=FALSE}
corrR <- function(X, Y) {
  n <- length(X)
...
In order to show how to use _corrR_, we give an example.
...
sigma <- 1;n <- 100000
u <- runif(n)
x <- sqrt(-2*(sigma^2)*log(1-u)) # F(x) = 1-exp(-x^2/(2*σ^2)), x>=0,σ>0
